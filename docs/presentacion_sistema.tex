...
\centering
\begin{tikzpicture}[node distance=2.2mm and 4mm, font=\tiny]
  \node[dato] (q) {$q$};
  \node[ent, below left=4mm and 1mm of q, fill=verde!12] (e1) {$\varepsilon_1$\\ \tiny$\mathbf v{=}1$ \textcircled{\tiny 4}};
  \node[ent, below right=4mm and 1mm of q, fill=verde!12] (e2) {$\varepsilon_2$\\ \tiny$\mathbf v{=}1$ \textcircled{\tiny 4}};
  \node[aser, below=5mm of q, text width=17mm] (aa) {hecho $a\in S(q)$\\ $\mu{=}1$ \textcircled{\tiny 5}};
  \node[chk, below left=4mm and 0mm of aa, text width=16mm] (c1) {$\pi(\varepsilon_1)$\\ \tiny$+0{,}5$ \textcircled{\tiny 4}};
  \node[chk, below right=4mm and 0mm of aa, text width=16mm] (c2) {$\pi(\varepsilon_2)$\\ \tiny$+0{,}5$ \textcircled{\tiny 4}};
  \node[chk, right=9mm of c2, densely dotted, text width=13mm] (cx) {resto\\ \tiny$0{,}05\cos$ \textcircled{\tiny 4}};
  \draw[fl, grisf] (q) -- node[et, left]{\textcircled{\tiny 1}\textcircled{\tiny 2}\textcircled{\tiny 3}} (aa);
  \draw[fl] (e1) -- (aa); \draw[fl] (e2) -- (aa);
  \draw[fl] (aa) -- (c1); \draw[fl] (aa) -- (c2);
  \draw[flok, very thick] (e1) to[out=-100,in=170] node[et, left]{\textcircled{\tiny 6} paseo} (c1);
  \draw[flmal, dashed] (e2) to[out=-30,in=90] node[et, right]{\textcircled{\tiny 7} salto} (cx);
  \node[et, below=2mm of c2, text width=48mm, align=center]
...
\begin{frame}{Estudio de caso: la misma consulta ante los tres sistemas}
\footnotesize
\emph{«Which film has the director who is older, God's Gift to Women or Aldri annet enn br\aa k?»}
--- oro: los dos filmes y sus directores (\textbf{Michael Curtiz}, \textbf{Edith Carlmar}).
\vspace{1mm}
\begin{columns}[T]
\column{0.34\textwidth}
\centering \textbf{\hippo{} (medido)}\\[0.5mm]
{\scriptsize
1. \bien{Aldri annet enn br\aa k}\\
2. \bien{God's Gift to Women}\\
3. \bien{Edith Carlmar}\\
4. \mal{Altid ballade}\\
5. \mal{Bedre enn sitt rykte}\\[0.5mm]
$\mathrm{R@}5=0{,}75$; $\mathrm{FC@}5=\mal{0}$}\\[0.5mm]
\scriptsize\emph{Deriva a una sola rama}: los
puestos 4--5 son \emph{otras películas de Carlmar}; Curtiz no se siembra
nunca.
\column{0.32\textwidth}
\centering \textbf{CatRAG (mecanismo)}\\[0.5mm]
\scriptsize Re-pondera las aristas del \emph{mismo} grafo de tripletes ante la
consulta (código no liberado). No puede crear la semilla ausente ni resolver
la identidad de menciones: hereda las islas y el presupuesto fijo por
consulta. En 2Wiki publica $\mathrm{FCR}=67{,}6$
($+1{,}5$ sobre su base): la re-ponderación \emph{sola} apenas repara la
cadena.
\column{0.34\textwidth}
\centering \textbf{\sys{} v4 (medido)}\\[0.5mm]
{\scriptsize
1. \bien{God's Gift to Women}\\
2. \bien{Michael Curtiz}\\
3. \bien{Aldri annet enn br\aa k}\\
4. \bien{Edith Carlmar}\\
5. Altid ballade\\[0.5mm]
$\mathrm{R@}5=1$; $\mathrm{FC@}5=\bien{1}$}\\[0.5mm]
\scriptsize El plan declara \emph{ambos} directores como huecos; la compuerta
y el pasaje propio siembran a Curtiz \emph{y} a Carlmar: recursos por cadena,
no por consulta.
\end{columns}
\vspace{1.5mm}
\scriptsize El contraste ilustra la tesis: re-ponderar un grafo pobre
(CatRAG) no sustituye a \emph{representar mejor} (aserciones canónicas) ni a
\emph{planificar la evidencia} (asignación por hueco).
\end{frame}
