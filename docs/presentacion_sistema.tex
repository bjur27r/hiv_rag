...
\begin{frame}{Estudio de caso: tres mecanismos, alineados etapa a etapa}
\centering\scriptsize
\emph{«Which film has the director who is older, God's Gift to Women or Aldri annet enn br\aa k?»}
--- oro: 2 filmes $+$ sus directores (\textbf{M. Curtiz}, \textbf{E. Carlmar}).\quad
\colorbox{grisf!15}{\tiny heredado de \hippo{} sin cambios}\ \
\colorbox{teja!15}{\tiny lo que cada sistema cambia}
\vspace{1mm}

\resizebox{0.99\textwidth}{!}{%
\begin{tikzpicture}[x=1mm, y=1mm, font=\tiny]
  % etiquetas de etapa (izquierda)
  \node[et, align=right, anchor=east] at (-2,42) {\textbf{ÍNDICE}\\ (grafo)};
  \node[et, align=right, anchor=east] at (-2,29) {\textbf{SIEMBRA}\\ $\mathbf v$};
  \node[et, align=right, anchor=east] at (-2,16) {\textbf{PASEO}\\ $+$ decisión};
  \node[et, align=right, anchor=east] at (-2,4)  {\textbf{RESULTADO}};
  % cabeceras
  \node[et] at (22,50)  {\textbf{\hippo{} (medido)}};
  \node[et] at (70,50)  {\textbf{CatRAG (su artículo)}};
  \node[et] at (118,50) {\textbf{\sys{} (medido)}};
  % ---------------- fila INDICE
  \node[caja, fill=teja!15, draw=teja, text width=42mm] (i1) at (22,42)
    {tripletes como \emph{aristas}; menciones \mal{sin resolver} $+$ $172$k aristas de sinonimia};
  \node[caja, fill=grisf!15, draw=grisf, text width=42mm] (i2) at (70,42)
    {\textbf{el mismo grafo} de tripletes\\ (no cambia nada en indexación)};
  \node[caja, fill=teja!15, draw=teja, text width=42mm] (i3) at (118,42)
    {\bien{distinto}: aserciones-\emph{nodo} ($\tau_a$, roles $n$-arios) sobre entidades \textbf{canónicas}; sin sinonimia};
  % ---------------- fila SIEMBRA
  \node[caja, fill=teja!15, draw=teja, text width=42mm] (s1) at (22,29)
    {frases de los $\le 4$ triples filtrados: cubrió \emph{una rama}\\ (Carlmar sí, \mal{Curtiz no})};
  \node[caja, fill=grisf!15, draw=grisf, text width=42mm] (s2) at (70,29)
    {\textbf{la misma siembra} $+$ anclas débiles $\epsilon$:\\ \mal{no puede crear la semilla ausente} (Curtiz sigue fuera)};
  \node[caja, fill=teja!15, draw=teja, text width=42mm] (s3) at (118,29)
    {\bien{distinta}: el \textbf{plan} declara ambos directores $\Rightarrow$ compuerta $\mathbf v{=}1$ en \emph{los dos} $+$ $0{,}5$ a sus pasajes propios};
  % ---------------- fila PASEO
  \node[caja, fill=teja!15, draw=teja, text width=42mm] (p1) at (22,16)
    {$T$ \emph{estática}: la masa difunde al vecindario denso (otras películas de Carlmar)};
  \node[caja, fill=teja!15, draw=teja, text width=42mm] (p2) at (70,16)
    {\textbf{su única novedad}: re-pondera aristas \emph{cercanas a las semillas} según $q$ --- pero Curtiz no está cerca de ninguna semilla};
  \node[caja, fill=teja!15, draw=teja, text width=42mm] (p3) at (118,16)
    {\bien{distinto}: $\mu_a(q)$ sobre \emph{todos} los hechos ($\mu{=}1$ los aprobados) $+$ salto por huecos $+$ \textbf{conjunto} que cubre el plan};
  % ---------------- fila RESULTADO
  \node[dato, text width=42mm] (r1) at (22,4)
    {top-5: 3 oros $+$ 2 filmes de Carlmar\\ $\mathrm{R@}5{=}0{,}75$; $\mathrm{FC@}5{=}\mal{0}$};
  \node[dato, text width=42mm] (r2) at (70,4)
    {publicado (mismo backbone):\\ $\mathrm{FCR}=67{,}6$ ($+1{,}5$ sobre su base)};
  \node[dato, fill=verde!8, text width=42mm] (r3) at (118,4)
    {top-4 $=$ los 4 oros\\ $\mathrm{R@}5{=}1$; $\mathrm{FC@}5{=}\bien{1}$};
...
\vspace{1mm}
\scriptsize Lectura por columnas: CatRAG solo cambia \emph{una} etapa (re-ponderar
el paseo) y hereda índice y siembra --- por eso su mejora es marginal
($+1{,}5$): re-ponderar no alcanza a una semilla que no existe. \sys{} cambia
\emph{las cuatro}: representación, identidad, siembra por plan y decisión de
conjunto.
\end{frame}
