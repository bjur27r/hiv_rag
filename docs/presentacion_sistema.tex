...
\begin{frame}{Arquitectura agéntica: deliberación acotada, control algorítmico}
\begin{columns}[T]
\column{0.5\textwidth}
\footnotesize
Tres \textbf{roles LLM} con contrato JSON estricto, orquestados por un flujo
\emph{determinista} (no hay bucle libre de agente):
\begin{itemize}\footnotesize
\item \textbf{Analista} (siempre; 1 llamada): comprensión $+$ plan $+$
  selección. Reglas duras: entidades no vistas $\Rightarrow[X]$; prohibido el
  conocimiento paramétrico; tope $=$ n.º de huecos.
\item \textbf{Reparador} (condicional, $\sim$20\,\%): rellena huecos con
  candidatos \emph{estructurales} ($\pi(\varepsilon)$) $+$ coseno del hueco.
\item \textbf{Empaquetador} (salvo comparación): conjunto de 5 que cubre el
  plan; guarda conservadora (el top-2 del paseo se preserva).
\end{itemize}
\textbf{Guardarraíles}: presupuesto $\le 3$ llamadas; degradación suave a
denso si nada se aprueba; salidas validadas (JSON, índices en rango); caché
por (modelo, consigna) $\Rightarrow$ determinismo y re-ejecución a coste
cero. \emph{El LLM decide dentro de contratos; el algoritmo conserva el
control} --- a diferencia de los agentes iterativos (IRCoT), la latencia y el
coste están acotados por diseño.
\column{0.48\textwidth}
\centering
\begin{tikzpicture}[x=1mm, y=1mm, font=\tiny]
  \node[caja, text width=30mm, minimum height=8mm] (orq) at (30,40) {\textbf{orquestador}\\ (flujo determinista)};
  \node[caja, fill=teja!12, draw=teja, text width=20mm] (r1) at (6,26)  {\textbf{analista}\\ plan $+$ $S(q)$};
  \node[caja, fill=teja!12, draw=teja, text width=20mm] (r2) at (30,26) {\textbf{reparador}\\ huecos};
  \node[caja, fill=teja!12, draw=teja, text width=20mm] (r3) at (54,26) {\textbf{empaquetador}\\ conjunto de 5};
  \node[dato, text width=17mm] (h1) at (6,12)  {diccionario\\ $\mathcal E$, $\pi(\varepsilon)$};
  \node[dato, text width=15mm] (h2) at (28,12) {grafo $G$\\ $+$ PPR};
  \node[dato, text width=17mm] (h3) at (52,12) {vectores\\ $\sigma_a$, $e(c)$};
  \draw[fl] (orq) -- (r1); \draw[fl] (orq) -- (r2); \draw[fl] (orq) -- (r3);
  \draw[fl, grisf] (r1) -- (h1); \draw[fl, grisf] (r1) -- (h3);
  \draw[fl, grisf] (r2) -- (h1); \draw[fl, grisf] (r2) -- (h3);
  \draw[fl, grisf] (r3) -- (h2);
  \node[et, text width=62mm] at (30,2)
    {los roles consultan las \emph{herramientas} (diccionario, grafo, vectores);\\
