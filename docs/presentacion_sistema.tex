...
\begin{tikzpicture}[x=1mm, y=1mm, font=\tiny]
  ...
  \node[dato]  (corpus) at (3,36)  {corpus $\mathcal D$};
  \node[caja]  (ext)    at (24,36) {extracción\\ $c\mapsto(E_c,A_c)$};
  \node[caja]  (dic)    at (49,36) {diccionario\\ $\mathcal E$, alias, $\pi(\varepsilon)$};
  \node[caja]  (gra)    at (74,36) {grafo $G$\\ $\mathcal C\cup\mathcal E\cup\mathcal A$};
  \node[dato]  (vec)    at (99,36) {vectores\\ $e(\tau_a)$, fichas, $e(c)$};
  \draw[fl] (corpus) -- (ext); \draw[fl] (ext) -- (dic);
  \draw[fl] (dic) -- (gra); \draw[fl] (gra) -- (vec);
  % ------- carril de consulta (abajo), pasos numerados
  \node[dato] (q)   at (3,10)   {$q$};
  \node[caja] (s1)  at (17,10)  {\textcircled{\tiny 1} $e(q)$;\\ $\sigma_a\ \forall a$};
  \node[caja] (s2)  at (33,10)  {\textcircled{\tiny 2} enlazador\\ $L(q)$};
  \node[caja, fill=teja!12, draw=teja] (s3) at (50,10) {\textcircled{\tiny 3} analista\\ plan, $S(q)$, huecos};
  \node[caja] (s4)  at (67,10)  {\textcircled{\tiny 4} siembra\\ $\mathbf v$};
  \node[caja] (s5)  at (81,10)  {\textcircled{\tiny 5}\textcircled{\tiny 6} $\tilde A$, PPR\\ $\mathbf p^{*}$};
  \node[caja, fill=teja!12, draw=teja] (s6) at (97,10) {\textcircled{\tiny 7} salto\\ dirigido};
  \node[caja, fill=teja!12, draw=teja] (s7) at (112,10) {\textcircled{\tiny 8} conjunto\\ final};
  \node[dato] (out) at (126,10) {top-5};
  \draw[fl] (q) -- (s1); \draw[fl] (s1) -- (s2); \draw[fl] (s2) -- (s3);
  \draw[fl] (s3) -- (s4); \draw[fl] (s4) -- (s5); \draw[fl] (s5) -- (s6);
  \draw[fl] (s6) -- (s7); \draw[fl] (s7) -- (out);
  % ------- conexiones verticales entre carriles (que usa cada paso)
  \draw[fl, grisf] (vec.south) to[out=-90,in=90] node[et, right, pos=0.35]{$e(\tau_a)$} (s1.north);
  \draw[fl, grisf] (dic.south) to[out=-90,in=90] node[et, left, pos=0.3]{alias, fichas} (s2.north);
  \draw[fl, grisf] (dic.south) to[out=-70,in=110] node[et, right, pos=0.6]{$\pi(\varepsilon)$} (s4.north);
  \draw[fl, grisf] (gra.south) to[out=-90,in=90] node[et, right, pos=0.45]{$A$} (s5.north);
  \draw[fl, grisf] (ext.south) to[out=-90,in=120] node[et, left, pos=0.35]{$\tau_a$ candidatos} (s3.north);
  \begin{scope}[on background layer]
    \node[fit=(corpus)(vec), draw=grisf, rounded corners=3pt, inner sep=2.5pt,
      label={[et]above left:{\textbf{indexación} (una vez; sin preguntas)}}] {};
    \node[fit=(q)(out), draw=grisf, rounded corners=3pt, inner sep=2.5pt,
      label={[et]below left:{\textbf{consulta} ($\sim$2 llamadas LLM; naranja = LLM)}}] {};
  \end{scope}
\end{tikzpicture}
\vspace{2mm}
\footnotesize Cada componente de indexación alimenta pasos concretos de la
consulta: los vectores de hechos a la afinidad \textcircled{\tiny 1}, el
diccionario al enlazador \textcircled{\tiny 2} y a la siembra
\textcircled{\tiny 4} (pasajes propios), las aserciones al analista
\textcircled{\tiny 3} (candidatos), y el grafo al paseo
\textcircled{\tiny 5}--\textcircled{\tiny 6}.
\end{frame}
